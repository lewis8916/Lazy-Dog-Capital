...
export function renderDrawEmail(data) {
  ...
  const attached = DRAW_REQUEST.attachments
    .map((a) => {
      const group = uploads.filter((u) => u.group === a);
      const files = group.length
        ? group
            .map(
              (u) =>
                `<div style="padding:2px 0 2px 20px;color:#5b6f69;font-size:12px;">${esc(u.filename)}${u.size ? ` · ${kb(u.size)}` : ""}</div>`
            )
            .join("")
        : `<div style="padding:2px 0 2px 20px;color:#b08b4f;font-size:12px;">Nothing attached</div>`;
      return `
        <div style="padding:6px 0;">
          <div style="color:${group.length ? TEAL : "#a4b0ab"};font-size:13px;font-weight:600;">
            ${group.length ? "&#10003;" : "&#9744;"} ${esc(a)}
          </div>
          ${files}
        </div>`;
