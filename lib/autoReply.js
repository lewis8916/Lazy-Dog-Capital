...
const EMAIL = "loans@lazydogcapital.com";
...
export function renderApplicationAck(d) {
  ...
  const body = `
    ...
    ${
      rec
        ? `<div style="margin-top:18px;padding-top:16px;border-top:1px solid #e6dfd1;">
             <div style="font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:${BRONZE};font-weight:700;margin-bottom:10px;">Your Signature Record</div>
             <table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
               ${kv([
                 ["Signed by", d.s1_sig || d.s1_name || "—"],
                 ["Received", `${rec.receivedAt} (UTC)`],
                 ["From IP", rec.ip],
               ])}
             </table>
             <div style="color:#7d8c87;font-size:11px;line-height:1.6;margin-top:10px;">
               You agreed to sign electronically. Keep this email as your record.
               For a paper copy, call ${PHONE}.
             </div>
           </div>`
        : ""
    }`;
