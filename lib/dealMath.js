...
export const REHAB_BUFFER_RATE = 0.1; // 10% contingency committed on top of rehab

/**
 * The borrower funds 10% of the purchase price, so the loan covers the rest of
 * the purchase plus the rehab budget and a 10% buffer on that budget — the
 * buffer is committed at closing, so it counts against the cap even if it is
 * never drawn. That figure, not the borrower's own project cost, is what gets
 * measured against 80% of the after-repair value.
 */
export function computeDeal({ price, rehab, arv }) {
  ...
  const buffer = Math.round(r * REHAB_BUFFER_RATE);
  const loanNeeded = p - down + r + buffer;
  const cap = Math.round(a * ARV_CAP_RATE);
  // What the borrower expects to spend. The buffer is contingency, so it is
  // deliberately left out of cost and spread.
  const totalCost = p + r;
